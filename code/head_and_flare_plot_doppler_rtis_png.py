...
for kp in xrange(5):
    ...
    for kf in xrange(5):#cs.vlt_sig.shape[1]):
        fig0, dpi = plotter(
            20*np.log10(np.abs((cs.vlt_sig[pslc, kf, csrslc]
        ...
        path = os.path.join(basedir, basefilename + '_recovered_doppler_rti_f{0}_p{1}.png')
        path = path.format(kf, pslc.start)

        fig0.savefig(path, dpi=savedpi, bbox_inches='tight', pad_inches=0.05, transparent=True)

        plt.close(fig0)

        fig1, dpi = plotter(
            20*np.log10(np.abs(cs.vlt_sig[pslc, kf, csrslc]/cs.noise_sigma)),
            cs.t[pslc], cs.r[csrslc]/1e3, pixelaspect=cspixelaspect*pixelaspect,
            exact_ticks=False, vmin=0, vmax=40, cmap=cmap,
            ylabel='Range (km)', clabel='SNR (dB)',
        )
        savedpi = int(round(300/float(dpi)))*dpi;
        path = os.path.join(basedir, basefilename + '_recovered_sparse_doppler_rti_f{0}_p{1}.png')
        path = path.format(kf, pslc.start)
        ...
        fig2, dpi = plotter(
            20*np.log10(np.abs(cs.vlt_noise[pslc, kf, csrslc]/cs.noise_sigma)),
            cs.t[pslc], cs.r[csrslc]/1e3, pixelaspect=cspixelaspect*pixelaspect,
            exact_ticks=False, vmin=0, vmax=40, cmap=cmap,
            ylabel='Range (km)', clabel='SNR (dB)',
        )
        savedpi = int(round(300/float(dpi)))*dpi;
        path = os.path.join(basedir, basefilename + '_recovered_noise_doppler_rti_f{0}_p{1}.png')
        path = path.format(kf, pslc.start)
