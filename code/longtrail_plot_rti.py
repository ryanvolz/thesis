...
pixelaspect=1
basefilename = 'longtrail_lowpass_up'
...
rslc = el.slice_by_value(mf.r, 90000, 101000)
csrslc = el.slice_by_value(cs.r, mf.r[rslc][0], mf.r[rslc][-1])
# want an integer ratio for pixel aspect between cs data and non-cs data
cspixelaspect = int(round(len(cs.r[csrslc])/float(len(mf.r[rslc]))))
# modify csrslc to make integer ratio exact
newstop = csrslc.stop + cspixelaspect*len(mf.r[rslc]) - len(cs.r[csrslc])
